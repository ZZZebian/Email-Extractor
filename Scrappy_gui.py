...
class EmailExtractor(QtWidgets.QWidget):
  ...
  def selectDirectory(self):
    # Open a dialog to select the top-level directory containing the HTML files
  ...
  def start(self):
    # Get the top-level directory containing the HTML files
    ...
    with open(os.path.join(save_dir, save_name), 'w', encoding='ISO-8859-1') as outfile:
      # Get the total number of HTML files to process
      ...
      for root, directories, filenames in os.walk(top_level_directory):
        # Iterate through the HTML files in the current directory
